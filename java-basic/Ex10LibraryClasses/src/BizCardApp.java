 ...
 public void doManage() { 
		...
		outer : while (true) {
			...
			case "3": // 검색 
				...
				// for (BizCard bizCard3 : bizCards) { // 전체 목록 순회
				...
				// for (BizCard searchedCard : searchedCards) {
...
//						BizCard bizCard2 = bizCards.get(i);
	...
	private void saveBizCards() {
		...
		try {
			fos = new FileOutputStream("bizcard.dat");
			oos = new ObjectOutputStream(fos);
			oos.writeObject(bizCards);
			System.out.println("파일에 명함 데이터를 저장했습니다.");
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
