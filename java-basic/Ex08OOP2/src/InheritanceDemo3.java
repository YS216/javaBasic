...
public class InheritanceDemo3 {
	...
	public static void main(String[] args) {
		...
		p.m(); // 오버라이딩된 메서드는 현재 타입에 따라 호출된다. (여기서는 TheParent3.m() 호출)
		TheChild3 c = new TheChild3();
		c.m(); // 오버라이딩된 메서드는 현재 타입에 따라 호출된다. (여기서는 TheChild3.m() 호출)
		
		///
		TheParent3 p2 = new TheChild3();
		p2.m(); // 참조 타입과 인스턴스 타입이 다를 겨우 재정이된 메서드 호출 기준 (항상 인스턴스 타입 기준)
		// 자바는 항상 인스턴스 타입을 따르게 되어있음. 
