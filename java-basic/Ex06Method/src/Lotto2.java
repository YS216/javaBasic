...
public class Lotto2 {
	...
	public static void main(String[] args) {
		...
		main_loop : while (true) {
			...
			case "1":
				// 당첨 예상번호 뽑기
				int[] numbers = selectWiningNumbers();
				// 평균 계산
	...
	public static int[] selectWiningNumbers() {
	...
	public static String selectTask() {
		System.out.println("********************");
		System.out.println("  1. 당첨 예상번호 뽑기");
	...
	public static int[] selectBasicNumbers() {
		int[] numbers = new int[6]; // 상위 클라스의 전달인자에는 정보가 없기 떄문에 해당 클래스에서 다시 배열 정의.
		for ( int i = 0; i < numbers.length; i++) {
			numbers[i] = (int)(Math.random() * 45) + 1; // 1~45

			// 중복 검사 반복문 ( 현재 뽑힌 i번쨰 번호와 0 ~ i-1 번째 번호 비교 )
			for (int j = 0; j < i; j++) {
