...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 파일 업로드 (multipart/from-data) 전송은 다음과 같이 데이터를 읽을 수 없다.
...
//		System.out.println("data 1 : " + data1);
		...
		if (ServletFileUpload.isMultipartContent(req) == false) { // multipart/form-data가 아니면
			resp.sendRedirect("07.file-upload.jsp"); // 파일 업로드 화면으로 이동
			return;
		}
		...
		//요청 정보를 파싱하고 개별 객체의 목록을 반환 (DixkFileItemFactory가 생산하는 객체가 FileItem 객체)
		List<FileItem> items = null;
		try {
			items = uploader.parseRequest(req); // 요청 데이터 일기 -> 분해 -> 객체에 저장 -> 목록에 담기
