...
public class RegisterServlet2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// Object members = application.getAttribute("members");
		// members에 회원 데이터 저장하는 코드
		members.add(member);
		
		// 3. 응답컨텐츠 생산 ( 여기서는 다른 작업 영역으로 regrect로 이동)
//		resp.setContentType("text/html;charset=utf-8");
//		PrintWriter out = resp.getWriter();
//		out.printf("<h1>[%s][%s][%s][%s]</h1>", memberId, passwd, confirm, email);
		
		resp.sendRedirect("/demoweb/home.action"); // 이 주소는 브라우저가 사용하는 주소이므로 애플리케이션 이름을 포함해야한다.
