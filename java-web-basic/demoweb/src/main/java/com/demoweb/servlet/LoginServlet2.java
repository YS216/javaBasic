...
public class LoginServlet2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 저장된 회원가입 정보에서 사용자 정보 조회 및 로그인 설정
		ServletContext application = req.getServletContext();
		ArrayList<MemberDto> members = (ArrayList<MemberDto>)application.getAttribute("members");
		for (MemberDto member : members) {
			if(memId.equals(member.getMemberId()) && passwd.equals(member.getPasswd())) { // 로그인 성공
				// 로그인 처리 - 세션에 로그인 정보 저장
				HttpSession session = req.getSession();
				session.setAttribute("loginuser", memId); // 세션에 데이터 저장
				
				// 4. 일련의 작업이 끝났으면 redirect로 이동
				resp.sendRedirect("/demoweb/home.action"); // sendRedirect는 메서드를 종료하지 않음
				return; // 메소드 종료
			}
		}
//		if (memId.equals("iamuser") && passwd.equals("1234")) { // 로그인 성공
//			// 로그인 처리 - 세션에 로그인 정보 저장
...
//			doGet(req, resp); // 다시 로그인 페이지로 foward 이동
//		}
